...
export default function MenuPage({ params }: PageProps) {
  ...
  useEffect(() => {
    const fetchMenu = async () => {
      try {
        ...
        const loadedItems: MenuItem[] = result.data.map(
          (item: Omit<MenuItem, "id"> & { id: string | number }) => ({
            ...item,
            id: String(item.id),
          })
        );
  ...
  const handleAdd = (item: MenuItem) => {
    if (item.isAvailable === false) {
      ...
      setErrorMessage(`${item.name} は品切れです。別の商品をお選びください。`);
      return;
    }
    ...
    setToastMessage("カートに追加しました。");
  };

  return (
    <main
      className="min-h-dvh bg-yellow-50 bg-[linear-gradient(0deg,rgba(160,120,60,0.08)_1px,transparent_1px),linear-gradient(90deg,rgba(160,120,60,0.06)_1px,transparent_1px),radial-gradient(circle_at_15%_25%,rgba(0,0,0,0.04),transparent_40%),radial-gradient(circle_at_85%_75%,rgba(0,0,0,0.03),transparent_50%)]"
      style={{ backgroundSize: "4px 4px, 4px 4px, 100% 100%, 100% 100%" }}
    >
      <div className="mx-auto w-full max-w-md px-3 py-4">
        <header className="mb-4">
          <h1 className="text-center text-3xl font-black tracking-wide">Osaki Cafe</h1>
          <p className="mt-1 text-center text-sm text-muted-foreground">お席番号: {tableId}</p>
        </header>
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              <div className="text-sm text-muted-foreground">
                {filteredItems.length === 0
                  ? "0-0 件表示"
                  : `${Math.min((currentPage - 1) * PAGE_SIZE + 1, filteredItems.length)}-${Math.min(
